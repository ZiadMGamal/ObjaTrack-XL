...
class ConfigValidator:
    ...
    def validate_strict(self) -> None:
        if not self.validate():
            raise ConfigurationError(
                f"Configuration validation failed with {len(self._errors)} error(s): " + "; ".join(self._errors)
            )

    def _validate_device(self) -> None:
        device = self._settings.device.lower()
        if device not in self.SUPPORTED_DEVICES:
            self._errors.append(f"Unsupported device '{device}'. Supported: {self.SUPPORTED_DEVICES}")
    ...
    def _validate_model(self) -> None:
        ...
        if model_path.suffix == ".pt" and model.name not in self.SUPPORTED_MODELS:
            self._warnings.append(f"Model '{model.name}' is not a standard YOLOv8 variant. Ensure it exists.")

        if model.confidence_threshold >= model.iou_threshold:
            self._warnings.append("Confidence threshold >= IOU threshold. This may reduce detection quality.")
    ...
    def _validate_source(self) -> None:
        ...
        if source.type not in self.SUPPORTED_SOURCES:
            self._errors.append(f"Unsupported source type '{source.type}'. Supported: {self.SUPPORTED_SOURCES}")
    ...
    def _validate_tracker(self) -> None:
        ...
        if tracker.type not in self.SUPPORTED_TRACKERS:
            self._errors.append(f"Unsupported tracker type '{tracker.type}'. Supported: {self.SUPPORTED_TRACKERS}")
    ...
    def _validate_thresholds(self) -> None:
        ...
        if model.confidence_threshold < 0.1:
            self._warnings.append("Very low confidence threshold may produce many false positives.")

        if model.confidence_threshold > 0.9:
            self._warnings.append("Very high confidence threshold may miss valid detections.")
